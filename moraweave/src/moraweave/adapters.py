...
class FasterWhisperAdapter:
    """One-window N-best adapter around faster-whisper and CTranslate2."""
    ...
    def decode(self, request: DecodeRequest) -> list[CandidateEvidence]:
        ...
        for candidate in rows:
            ...
            candidate_score = candidate.avg_logprob if candidate.avg_logprob is not None else -math.inf
            current_score = (
                current.avg_logprob
...
def _audio_slice(request: DecodeRequest) -> str | tuple[Any, int]:
    ...
    try:
        ...
        raise RuntimeError("qwen-asr span decoding requires librosa") from exc
    ...
    if len(waveform) == 0:
        raise ValueError("Qwen3-ASR span contains no audio")
...
class Qwen3ASRAdapter:
    """Official qwen-asr 0.0.6+ second-ear adapter."""
            ...
            candidates.append(
                CandidateEvidence(
                    ...
                    metadata={
                        ...
                        "candidateMultiplicity": "single-transcript-per-input",
                    },
                )
            )
        if not candidates:
            raise RuntimeError("Qwen3-ASR returned no transcript")
        return candidates[: request.hypotheses]
...
class Qwen3ForcedAlignerAdapter:
    ...
    def align(self, request: DecodeRequest, *, text: str) -> list[AlignedToken]:
        ...
        results: Any = self.model.align(
            audio=_audio_slice(request),
            text=text,
            language=qwen_language_name(request.language),
        )
        rows = results[0] if isinstance(results, list) else results
