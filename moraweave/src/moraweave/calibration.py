...
class CalibrationProfile:
    """Versioned post-hoc scalar calibration for one evidence stream.

    The profile is intentionally tiny so it can be fitted on a held-out Japanese
    calibration set and distributed without bundling model weights or audio.
    """
    ...
    direction: float = 1.0
    input_kind: Literal["score", "probability"] = "score"
    version: str = "1"
    ...
    def __post_init__(self) -> None:
        ...
        if self.input_kind not in {"score", "probability"}:
            raise ValueError("input_kind must be score or probability")
    ...
    def transform(self, value: float | None) -> float | None:
        ...
        if self.input_kind == "probability":
            probability = _clip_probability(numeric)
            numeric = math.log(probability / (1.0 - probability))
            z_score = self.direction * numeric / self.temperature
        else:
            z_score = self.direction * (numeric - self.center)
            z_score /= self.scale * self.temperature
        return _clip_probability(_sigmoid(z_score))
...
def robust_profile(
    ...
    *,
    ...
    """Build a robust fallback profile using median and MAD.

    This is a fallback for uncalibrated score domains. A persisted held-out profile
    is preferred because even robust set normalization is not a substitute for
...
def calibrate_values(
    ...
    *,
    ...
    """Calibrate evidence without fragile min-max normalization."""
    ...
    probability_like = all(0.0 <= value <= 1.0 for value in finite)
    if probability_like:
        return [
            None
            if value is None or not math.isfinite(float(value))
...
class ScoreRankFeatures:
    """Architecture-independent beam features for a confidence baseline."""
...
def score_rank_confidence(features: ScoreRankFeatures) -> float:
    """Deterministic SR-CEM-inspired baseline, not a learned reproduction."""
    ...
    if features.hypothesis_count <= 1:
        ...
        rank_fraction = (
            features.hypothesis_count - features.rank
...
def fit_temperature(
    ...
    *,
    ...
    """Grid-search a scalar temperature using held-out binary NLL."""

    if len(probabilities) != len(labels) or not probabilities:
        raise ValueError("probabilities and labels must have equal non-zero length")
    grid = tuple(candidates or [0.35, 0.5, 0.7, 0.85, 1.0, 1.2, 1.5, 2.0, 3.0])
    logits = [
        math.log(_clip_probability(probability) / (1 - _clip_probability(probability)))
