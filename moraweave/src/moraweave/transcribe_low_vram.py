...
from .longform import LongFormConfig
from .low_vram import transcribe_low_vram


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m moraweave.transcribe_low_vram",
        description="Single-model complete Japanese transcription for constrained local GPUs.",
    )
    ...
    parser.add_argument("--acoustic-cache", default="data/acoustic_cache.sqlite3")
    ...
    config = LongFormConfig(
        ...
        qwen_second_ear=False,
    )
    document = transcribe_low_vram(
        args.audio,
        output_dir=args.output_dir,
        config=config,
        acoustic_cache_path=args.acoustic_cache,
    )
