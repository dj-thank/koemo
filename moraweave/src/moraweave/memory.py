...
class HashedNgramMemory:
    """SQLite memory that stores counts and hashes, not source sentences."""
        ...
        *,
        ...
        for text in texts:
            ...
            if not value:
                continue
            rows_seen += 1
            input_hasher.update(value.encode("utf-8"))
        ...
        with self.connection:
            for (digest, size), count in counter.items():
                self.connection.execute(
                    """
                    INSERT INTO ngrams(namespace, digest, n, count)
                    VALUES (?, ?, ?, ?)
                    ON CONFLICT(namespace, digest)
            ...
            self.connection.execute(
                """
                INSERT OR REPLACE INTO builds(build_id, asset_id, input_digest, rows_seen)
                VALUES (?, ?, ?, ?)
                """,
        ...
        return {
            ...
            "rowsSeen": rows_seen,
            "uniqueHashedNgrams": len(counter),
        }
...
class TeacherProbabilityCache:
    ...
    def get(self, key: TeacherCacheKey) -> dict[str, float] | None:
        row = self.connection.execute(
            "SELECT payload_json FROM teacher_cache WHERE key_digest = ?", (key.digest(),)
        ).fetchone()
        if row is None:
            return None
