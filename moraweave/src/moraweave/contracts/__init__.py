...
class ObservedTranscript:
    ...
    @classmethod
    def create(cls, *, selected: RankedCandidate, ranked: list[RankedCandidate], uncertainty_spans: list[dict[str, Any]], source_audio_sha256: str | None = None) -> "ObservedTranscript":
...
class NormalizedTranscript:
    ...
    @classmethod
    def attach(cls, observed: ObservedTranscript, *, text: str, mode: Literal["deterministic", "rank-only", "guarded-rewrite"], selected_candidate_id: str | None = None, rejected_edits: tuple[str, ...] = ()) -> "NormalizedTranscript":
        observed.verify()
        if selected_candidate_id is not None and selected_candidate_id not in {candidate.candidate_id for candidate in observed.candidates}:
            raise ValueError("normalization selected a candidate outside the observed lattice")
        return cls(text=text, observed_evidence_sha256=observed.evidence_sha256, mode=mode, selected_candidate_id=selected_candidate_id, rejected_edits=rejected_edits)


__all__ = [
    "EvidenceName", "MoraUnit", "CandidateEvidence", "GateDecision", "RankedCandidate",
    "ObservedTranscript", "NormalizedTranscript", "canonical_json", "sha256_json"
]
